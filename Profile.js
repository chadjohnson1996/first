...
export default class Profile extends React.Component {
    ...
    // TODO: Rhonda, this is something to review
    uploadHandler = () => {
        // you can build a file name however you want, this is just for example
        // it probably makes sense to build the file names with something uniquely identifying to the
        // user so that you can seperate who owns what files, but it is up to you
        // this is honestly a pretty hacky way of doing it, so feel free to do whatever
        var newFileName = this.props.location.state.email + '_' + this.state.file.name

        // the file.name property is read only, so we have to create a new file on
        // a form data and get it to change the name
        var formData = new FormData();
        formData.append('file', this.state.file, newFileName);
        var file = formData.get('file')
        // post the new file with the helper library
        axiosFileupload('/file_upload', file);
    }
    ...
    render() {
        ...
        return (
            <div className="profile-back">
                ...
                <input type='file' onChange={this.fileChangedHandler} name='test_file' placeholder='image upload'></input>
